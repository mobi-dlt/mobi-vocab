...
Object.keys(schemas).forEach((schemaName) => {
  const schema = schemas[schemaName];
  const goodExample = path.resolve(
    __dirname,
  ...
  if (fs.existsSync(goodExample)) {
    ...
    try {
      const good = JSON.parse(fs.readFileSync(goodExample));
      ...
      console.warn(
        `could not load good examples for ${schemaName}, with schema id:`,
        schema.$id
      );
...
console.log(
  'Validation results:\n',
  validationResults.filter((vr) => vr.status !== 'PASSED')
);
